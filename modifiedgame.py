...
class Brick(GameObject):
    COLORS = {1: '#8D472A', 2: '#7D6A66', 3: '#2C190B'}
    ...
    def blink(self, count=3):
        if count > 0:
            # Mengubah warna menjadi hitam sebagai efek berkedip
...
class Game(tk.Frame):
    def __init__(self, master):
        ...
        self.width = 610
        self.height = 400
        self.canvas = tk.Canvas(self, bg='#A08269',
                                width=self.width,
                                height=self.height)
        self.canvas.pack()
        self.pack()
    ...
    def restart_game(self):
        # Menghapus tombol restart
        if self.restart_button is not None:
            self.restart_button.destroy()
            self.restart_button = None
        
        # Untuk reset lives dan score
        self.lives = 3
        self.score = 0
    ...
    def show_restart_button(self):
        ...
        self.restart_button = tk.Button(self, text="Restart", font=('Arial', 14),
                                        command=self.restart_game, bg="#black", fg="black")
